...
Most code is borrowed from LightDB
...
class LightDB(dict):
    ...
    def load(self):
        return (
            json.load(open(self.location, "r")) if os.path.exists(self.location) else {}
        )

    def save(self):
        json.dump(self, open(self.location, "w+"), ensure_ascii=False, indent=4)
        return True
    ...
    def get_full(self):
        return defaultdict(list, dict(self))
